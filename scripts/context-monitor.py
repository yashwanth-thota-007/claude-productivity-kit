...
def get_pomodoro_display(session_id: str) -> str:
    """Return a live 🍅 countdown from pomodoro-state.json written by the menu bar app."""
    try:
        if not POMODORO_STATE.exists():
            return ""
        state = json.loads(POMODORO_STATE.read_text())
        if state.get("session_id") != session_id:
            return ""
        remaining = float(state["end_ts"]) - time.time()
        if remaining <= 0:
            return " \033[90m|\033[0m \033[31m🍅 Done!\033[0m"
        ...
        if remaining <= 5 * 60:
            ...
            color = "\033[32m"
        return f" \033[90m|\033[0m {color}🍅 {mins}:{secs:02d}\033[0m"
    except Exception:
        return ""
...
def main():
    try:
        ...
        pomodoro_display = get_pomodoro_display(session_id)
        ...
        status_line = f"{model_display} \033[93m📁 {directory}\033[0m 🧠 {context_display}{session_metrics}{pomodoro_display}"
